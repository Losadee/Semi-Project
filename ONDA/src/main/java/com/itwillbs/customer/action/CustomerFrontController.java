...
// 뽑은 가상주소		  =>			 /insertForm.me
		...
		if(strpath.equals("/CustomerInsertForm.cu")) {	
			forward = new ActionForward();
			forward.setPath("./customer/insertForm.jsp");
			forward.setRedirect(false);
			
		} else if(strpath.equals("/CustomerInsertPro.cu")) {
			action = new CustomerInsertPro();
			try {
		...
		}  else if(strpath.equals("/CustomerKakaoLoginPro.cu")) {
			action = new CustomerKakaoLoginPro();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();
